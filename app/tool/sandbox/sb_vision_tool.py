...
_VISION_DESCRIPTION = """\
基于沙箱的视觉工具，允许 agent 使用 see_image 操作读取沙箱内的图片文件。
* 仅支持 see_image 操作，参数是 /workspace 下图片的相对路径。
* 图片将被压缩并转换为 base64，用于后续上下文。
* 支持的格式：JPG、PNG、GIF、WEBP。最大大小：10MB。
"""
    ...
    async def execute(
        ...
        try:
            ...
            try:
                ...
                if file_info.is_dir:
                    return self.fail_response(f"Path '{cleaned_path}' is a directory, not an image.")
            except Exception:
                return self.fail_response(f"Image file not found: '{cleaned_path}'")

            if file_info.size > MAX_IMAGE_SIZE:
                return self.fail_response(
                    f"Image file '{cleaned_path}' too large ({file_info.size / (1024*1024):.2f}MB), "
                    f"max allowed {MAX_IMAGE_SIZE / (1024*1024)}MB."
                )
            ...
            if not mime_type or not mime_type.startswith("image/"):
                ...
                if not mime_type:
                    return self.fail_response(
                        f"Unsupported image format: '{cleaned_path}'. Supported: JPG, PNG, GIF, WEBP."
                    )
            ...
            if len(compressed_bytes) > MAX_COMPRESSED_SIZE:
                return self.fail_response(
                    f"Image file '{cleaned_path}' still too large after compression "
                    f"({len(compressed_bytes) / (1024*1024):.2f}MB), max allowed {MAX_COMPRESSED_SIZE / (1024*1024)}MB."
                )
            ...
            return self.fail_response(f"Error executing vision action: {str(e)}")
